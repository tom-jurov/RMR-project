    ...
    for (auto& sp : submaps_) {
        ...
        if (dis < candidate_distance_th_) {
            ...
            std::cout << "taking " << current_frame_->keyframe_id_ << " with " << sp.first
                      << ", last submap id: " << last_submap_id_ << std::endl;
...
void LoopClosing::MatchInHistorySubmaps() {
    ...
    for (const size_t& can : current_candidates_) {
        ...
        if (mr->AlignG2O(pose_in_target_submap)) {
            ...
            std::cout << "adding loop from submap " << can << " to " << last_submap_id_ << std::endl;
...
void LoopClosing::Optimize() {
    ...
    for (auto& ep : loop_edges) {
        if (ep.second->chi2() < loop_rk_delta_) {
            std::cout << "loop from " << ep.first.first << " to " << ep.first.second
                      << " is correct, chi2: " << ep.second->chi2() << std::endl;
            ...
            std::cout << "loop from " << ep.first.first << " to " << ep.first.second
                      << " is invalid, chi2: " << ep.second->chi2() << std::endl;
    ...
    std::cout << "loop inliers: " << inliers << "/" << loop_constraints_.size() << std::endl;
