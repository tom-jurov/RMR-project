...
void Mapping2D::UndistortScan(LaserMeasurement &measurement){
    ...
    for (int i = 0; i < measurement.numberOfScans; ++i) {
        // compute interpolation factor alpha using timestamps
        double alpha = (static_cast<double>(measurement.Data[i].timestamp) - t_start) / dt;
        alpha = std::clamp(alpha, 0.0, 1.0); // ensure within [0,1]

        Sophus::SE2d interp_relative = Sophus::SE2d::exp(alpha * relative.log());

        double r = measurement.Data[i].scanDistance;
        double angle_rad = measurement.Data[i].scanAngle * M_PI / 180.0;
        Eigen::Vector2d point(std::cos(angle_rad) * r, std::sin(angle_rad) * r);

        // transform by interpolated relative motion
...
void Mapping2D::ExpandSubmap() {
    ...
    std::cout << "create submap " << current_submap_->GetId()
              << " with pose: " << current_submap_->GetPose().translation().transpose() << ", "
              << current_submap_->GetPose().so2().log()<<std::endl;
}
