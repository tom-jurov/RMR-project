...
bool MRLikelihoodField::AlignG2O(SE2& init_pose) {
    ...
    /// 成功匹配的话，打印一些信息
    for (int l = 0; l < levels_; ++l) {
        std::cout << "level " << l << " inliers: " << num_inliers_[l] << ", ratio: " << inlier_ratio_[l] << std::endl;
    }
...
bool MRLikelihoodField::AlignInLevel(int level, SE2& init_pose) {
    ...
    for (int i = 0; i < source_->numberOfScans; ++i) {
        float r = source_->Data[i].scanDistance * 0.001f;
        if (r <= 0) continue;
        if (r > range_th) continue;

        float angle = source_->Data[i].scanAngle * static_cast<float>(M_PI / 180.0f);
        if (angle < -M_PI * 150 / 180.0 ||
            angle > M_PI * 150 / 180.0) {
            continue;
        }
